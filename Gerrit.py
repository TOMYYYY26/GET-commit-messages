...
class GerritClient:
    ...
    def get_project_changes(self, project_name: str, limit: int = 500) -> list:
        """获取项目的所有变更列表"""
        url = f"http://{self.host}/a/changes/?q=project:{project_name}&n={limit}"
    ...
    def _analyze_bug_with_ai(self, diff_content: str, change_subject: str) -> Tuple[bool, str, str]:
        """
        使用阿里云百炼(qwen-plus)模型分析bug类型和描述---改为qwen2.5-coder-7b-instruct
        ...
        # 构造提示词
        prompt = f"""
        请严格分析以下代码变更是否是一个真实的bug修复，并按要求回答。
        ...
        判断标准:
        1. 如果是格式调整、注释修改、import优化等非功能性变更，判断为"否"
        ...
        try:
            response = self.ai_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "你是一个严谨的代码审查助手，需要严格分析代码变更是否是bug修复。"},
    ...
    def filter_bug_fixes(
        self,
        changes: List[Dict],
        bug_keywords: List[str] = ["Bug", "Fix", "Defect"]
    ) -> List[Dict]:
        """
        严格的两步筛选：
        1. 先通过关键词筛选变更
    ...
    def download_bugfix_patches(
        ...
        if bug_keywords is None:
            bug_keywords = ["Bug", "Fix", "Defect", "错误", "修复"]
...
if __name__ == "__main__":
    client = GerritClient("gerrit_host")
    
    # 执行严格模式下载
    client.download_bugfix_patches(
        project_name="your_project",
        bug_keywords=["Bug", "BUG"]
    )
